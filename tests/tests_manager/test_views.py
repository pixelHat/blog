...
class CommentViews(TestCase):
    ...
    def test_show_comment(self):
        client = Client()
        response = client.get('/manager/comments/')
    ...
    def test_read_comment(self):
        client = Client()
        response = client.get('/manager/comments/read/', {'id': 1})
    ...
    def test_add_invalid_comment_to_waiting_list(self):
        client = Client()
        response = client.get('/manager/comments/add/waitList/', {'id': 10})
        self.assertJSONEqual(response.content, {'ok': False})

    def test_reply_comment(self):
        models.User.objects.create(name='user', email='admin@admin.com',
                                   image=IMAGE)
        client = Client()
        data = {'comment_id': 1, 'text': 'reply comment'}
        response = client.get('/manager/comments/reply/', data)
        self.assertJSONEqual(response.content, {'ok': True})
        self.assertEqual(models.Reply.objects.count(), 1)
